-- Settle in: sorting what an import brought, fast, in bulk, with an Undo for every choice.
--   * imports.settle: progress through the checklist ({steps: {key: {done_at, n}}}).
--   * settle_ops: each bulk choice, with every changed row's previous values (for Undo).
--   * settle_apply(batch, op, args): one bulk choice over rows of that import only:
--       someday     {ids}          the on-hold Someday tag (a group goes too once all its steps have)
--       complete    {ids}          completed now
--       drop        {ids}          dropped now (never deleted)
--       unflag      {ids}
--       clear_due   {ids}
--       plan        {ids, at}      due cleared, planned at `at`
--       project_status {ids, status}   active | on_hold | completed | dropped
--       review_dates   {items: [{id, at}]}  next review dates
--       drop_tags   {ids}          tags' status dropped
--     → {op_id, changed}
--   * settle_undo(op_id): puts every row back as it was.
alter table public.imports add column settle jsonb not null default '{}' check (jsonb_typeof(settle) = 'object');

create table public.settle_ops (
...
create index on public.settle_ops (import_id, created_at desc);
alter table public.settle_ops enable row level security;
create policy "owner select" on public.settle_ops for select to authenticated using ((select auth.uid()) = user_id);
create policy "owner insert" on public.settle_ops for insert to authenticated with check ((select auth.uid()) = user_id);
create policy "owner update" on public.settle_ops for update to authenticated using ((select auth.uid()) = user_id) with check ((select auth.uid()) = user_id);
create trigger settle_ops_no_delete before delete on public.settle_ops for each row execute function public.forbid_delete();
...
begin
  ...
  if op in ('someday', 'complete', 'drop', 'unflag', 'clear_due', 'plan') then
    ...
    if op = 'someday' then
      ...
      select coalesce(jsonb_agg(jsonb_build_object('t', 'task', 'id', t.id, 'completed_at', t.completed_at, 'dropped_at', t.dropped_at, 'flagged', t.flagged, 'due_at', t.due_at, 'planned_at', t.planned_at)), '[]')
...
revoke execute on function public.settle_apply(uuid, text, jsonb, uuid) from public, anon;
grant execute on function public.settle_apply(uuid, text, jsonb, uuid) to authenticated, service_role;
revoke execute on function public.settle_undo(uuid, uuid) from public, anon;
grant execute on function public.settle_undo(uuid, uuid) to authenticated, service_role;
