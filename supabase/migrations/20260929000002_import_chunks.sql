-- Big OmniFocus libraries are imported in chunks (a few thousand actions per call keeps each
-- request well inside the time limit). import_omnifocus(..., batch) adds a chunk to an existing
-- import, so one Undo still takes back the whole import. During an import the per-tag history
-- lines are skipped (each item still gets its "created" entry).
...
drop function public.import_omnifocus(jsonb, boolean, uuid);
...
revoke execute on function public.import_omnifocus(jsonb, boolean, uuid, uuid) from public, anon;
grant execute on function public.import_omnifocus(jsonb, boolean, uuid, uuid) to authenticated, service_role;
