-- Full Review: a suggestion can carry steps ("break it down"): {steps: [titles], steps_in_order?}. Submit
-- (review_apply) adds them under the action, after any it has, for keep / someday; Undo (review_undo)
-- drops them again (dropped, never deleted) and puts "do in order" back.
