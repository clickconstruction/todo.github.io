...
export function eventsTools({ localDate, zonedToIso }) {
  ...
  const shape = (e, tz, projectName = () => null) => {
    ...
    return {
      ...
      location: e.location || undefined, project: projectName(e.project_id) || undefined, url: e.url || undefined, notes: e.notes || undefined,
...
actions: list {from?, to?} (local dates; default today to 60 days out) · add {title, start, end?, all_day?, location?, notes?, url?, project?} or add {items: [{…}, …]} for several in one call · update {id, …the same fields} · remove {id} (archived, never deleted) · restore {id}.
Dates are the user's local time: "YYYY-MM-DD" makes an all-day event (end = its last day, inclusive; omitted = one day); "YYYY-MM-DDTHH:MM" a timed one (end defaults to an hour later).`,
    inputSchema: { type: 'object', properties: {
      ...
      items: { type: 'array', description: 'add: several events at once', items: { type: 'object', properties: {
        title: { type: 'string' }, start: { type: 'string' }, end: { type: 'string' }, all_day: { type: 'boolean' }, location: { type: 'string' }, notes: { type: 'string' }, url: { type: 'string' }, project: { type: 'string' } }, required: ['title', 'start'] } },
    } },
    async run(api, a) {
      ...
      if (action === 'remove' || action === 'restore') {
        ...
        return { ...shape(e, tz, projectName), next: action === 'remove' ? 'Removed (archived). events restore with this id brings it back.' : 'Back on the calendar.' };
      }
      // list
      ...
      return { from, to, events: rows.map((e) => shape(e, tz, projectName)) };
    },
  }];
}
