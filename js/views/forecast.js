...
export function forecastData() {
  ...
  return { today, tasks, overdue, plannedPast, days, future };
}
...
export function viewForecast(selected = 'today') {
  const { today, tasks, overdue, plannedPast, days, future } = forecastData();
  const pastCount = overdue.length + plannedPast.length;
  ...
  const strip = [
    ...
    ...days.map((d, i) => {
      ...
      const n = it.due.length + it.planned.length;
