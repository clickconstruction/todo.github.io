...
//   Ranking: due today or overdue > flagged > planned for today or earlier > serves an active goal >
...
export function rankNow(tasks, { now = new Date(), endOfToday, minutes = 0, energy = '', inContext = () => true, projects = [], goals = [], limit = 5 } = {}) {
  ...
  const pool = tasks.filter((t) => inContext(t) && fits(t) && energyOk(t));
  const scored = pool.map((t) => {
    ...
    if (t.planned_at && new Date(t.planned_at) <= end) { score += 300; reasons.push({ kind: 'planned', text: 'planned today' }); }
